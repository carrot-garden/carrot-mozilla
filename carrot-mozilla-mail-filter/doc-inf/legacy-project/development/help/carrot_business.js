...
//	flag that controls service <-> dom instaniation
var carrot_garden_is_service = true;
...
//			carrot_garden_compInfo.topicAppStartup, 
		...
		if( ! carrot_garden_is_service ) {
			logMessage( 'use in DOM' );
			catyMgr.addCategoryEntry(
				carrot_garden_compInfo.topicJSGlobalProprty, 
				carrot_garden_compInfo.serviceName, 
				carrot_garden_compInfo.serviceContractID, 
				true, true);
		}
...
//			carrot_garden_compInfo.topicAppStartup, 
 ...
 function carrot_garden_classService() { 
	...
	var flags;
	if( carrot_garden_is_service ) {
		logMessage('use as single service');
		flags = nsIClassInfo.SINGLETON;
	} else {
		logMessage('use as new dom instance each time new window is opened');
 ...
 */
	...
	this.loadScriptLibrary = function classService_loadScriptLibrary ( libName, scopeObject ) {
		switch ( libName ) {
			case ( 'jslib' ): 
				this.loadScript( "chrome://jslib/content/jslib.js", scopeObject ); break;
//			case ( 'jslib_file' ): 
//				this.loadScript( "chrome://jslib/content/io/file.js", scopeObject ); break;
			default: break;
		}
	}
	...
	this.initService = function classService_initService () {
		...
		try {
			...
			nsReportError( 'carrot_garden: could not load library: ' + libList[k] );
