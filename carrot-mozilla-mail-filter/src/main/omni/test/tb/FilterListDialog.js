...
var gStatusFeedback = {
	...
	startMeteors : function() {
		...
		runButton.setAttribute("accesskey", runButton
				.getAttribute("stopaccesskey"));

		if (!this.progressMeterVisible) {
			document.getElementById('statusbar-progresspanel').removeAttribute(
					'collapsed');
			this.progressMeterVisible = true;
		}

		document.getElementById("statusbar-icon").setAttribute("mode",
				"undetermined");
	},
 ...
 * 
 * @param aFolder
 *            the nsIMsgFolder that was selected
 */
...
function setFolder(msgFolder) {
	...
	// Calling getFilterList will detect any errors in rules.dat, backup the
	...
	if (CanRunFiltersAfterTheFact(msgFolder.server)) {
		...
		document.getElementById("runFiltersFolder").setAttribute("hidden",
				"true");
		document.getElementById("runFiltersButton").setAttribute("hidden",
				"true");
		document.getElementById("folderPickerPrefix").setAttribute("hidden",
				"true");
	}
...
function onDeleteFilter() {
	...
	if ((prefBranch.getBoolPref("mailnews.filters.confirm_delete"))
			&& (promptSvc.confirmEx(window, null, bundle
					.getString("deleteFilterConfirmation"),
					promptSvc.STD_YES_NO_BUTTONS, '', '', '', bundle
							.getString('dontWarnAboutDeleteCheckbox'),
					checkValue)))
		return;
...
function viewLog() {
	...
	window.openDialog("chrome://messenger/content/viewLog.xul", "FilterLog",
	...
	if (runButton.getAttribute("label") == runButton.getAttribute("stoplabel")) {
		...
		var result = promptSvc
				.confirmEx(
						...
						continueButtonLabel, stopButtonLabel, null, null, {
							value : 0
						});
...
function runSelectedFilters() {
	...
	var list = document.getElementById("filterList");
	var index = 0;
	for ( var item in list.selectedItems) {
		filterList.insertFilterAt(index++, item._filter);
	}

	filterService.applyFiltersToFolders(filterList, folders,
...
function rebuildFilterList(aFilterList) {
	...
	for (i = 0; i < aFilterList.filterCount; i++) {
		...
		// care
		// about clicks on the enabledCell. However, attaching to that item
		// doesn't
...
function updateButtons() {
	...
	var filter = currentFilter();
	// "edit" only enabled when one filter selected or if we couldn't parse the
	// filter
	...
	var upDisabled = !(oneFilterSelected && list.selectedItems[0] != list.childNodes[1]);
	...
	var downDisabled = !(oneFilterSelected && list.currentIndex < list
			.getRowCount() - 1);
	document.getElementById("reorderDownButton").disabled = downDisabled;
}

/**
 * Given a selected folder, returns the folder where filters should be defined
 * (the root folder except for news) if the server can accept filters.
 * 
...
/**
 * if the selected server cannot have filters, get the default server if the
 * default server cannot have filters, check all accounts and get a server that
 * can have filters.
 * 
...
function getServerThatCanHaveFilters() {
	...
	// that can have filters
		...
		for (index = 0; index < numServers; index++) {
			var currentServer = allServers.GetElementAt(index).QueryInterface(
...
function getFirstFolder(msgFolder) {
	...
	try {
		...
		if (msgFolder.server.type != "nntp") {
			...
			var inboxFolder = msgFolder
					.getFolderWithFlags(nsMsgFolderFlags.Inbox);
			if (inboxFolder)
				return inboxFolder;
			else
			...
			// XXX TODO: For news, we should find the 1st group/folder off the
			// news
			// groups. For now use server.
			return msgFolder;
	} catch (ex) {
		dump(ex + "\n");
	}
