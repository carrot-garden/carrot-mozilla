...
function editInstallManifest()
{
  ...
  if(f) {
    ...
    var w = window.openDialog("chrome://extensiondev/content/install-edit.xul", "installedit", "all=no,dialog=yes,scrollbars=yes,resizable=yes", f.path);
  }
}
...
function determineXPIFilename(wd, mf, cm, cb)
{
  ...
  var xpiFilename = findExistingXPI(wd);
  if(!xpiFilename) {
...
function initExtraFiles(folder, mf, cb)
{
  ...
  mf = mf || getInstallManifestFromDir(folder);
  var jarDirs = {};
  var jarFiles = [];
...
function getRegKey(root, key, name)
{
  ...
  if ("@mozilla.org/windows-registry-key;1" in Components.classes) {
    ...
    wss = Components.classes["@mozilla.org/winhooks;1"].getService(Components.interfaces.nsIWindowsRegistry);
    return wss.getRegistryEntry(wss[root], key, name);
  }
  else if ("@mozilla.org/browser/shell-service;1" in Components.classes) {
    wss = Components.classes["@mozilla.org/browser/shell-service;1"]
