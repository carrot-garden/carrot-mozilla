  ...
  try {
    ...
    for(i=0; i<versionProps.length; i++) {
      if(manifest[versionProps[i]]) {
        ...
        for(var j=0; j<manifest[versionProps[i]].length; j++) {
            // the Description
...
function getAppInfo (prop) {
  var info;
  if("@mozilla.org/xre/app-info;1" in Components.classes) {
    // running under Mozilla 1.8 or later
    info = Components.classes["@mozilla.org/xre/app-info;1"]
                   .getService(Components.interfaces.nsIXULAppInfo)[prop];
  } else {
    try {
      info = Components.classes["@mozilla.org/preferences-service;1"]
...
{
  var appId = getAppInfo('ID');
  var appExtVer = getAppInfo('version');
